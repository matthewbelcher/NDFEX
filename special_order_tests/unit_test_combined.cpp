...
void test_ioc_iceberg_order() {
    ...
    test_ladder.addOrder(order_id, md::SIDE::BUY, total_quantity, 100, flags, display_quantity);
    ...
    assert_equal(50, test_ladder.subscriber.calculateTotalFilled(order_id), "Fill quantity should match available liquidity");
    
    // Verify the order was deleted (IOC behavior) and order that was completely filled was also deleted
    assert_equal(2, test_ladder.subscriber.delete_order_count, "Order should be deleted (IOC behavior)");
...
void test_aon_iceberg_order() {
    ...
    assert_equal(100, test_ladder.subscriber.calculateTotalFilled(order_id), "Total filled quantity should match order quantity");
}
